...
async function startAutonomousLoop() {
  ...
  const prompt = `
    ...
    Instructions:
    - Choose an action: "search_google", "visit_url", "type_text", or "click_element".
    - "type_text" needs { "selector": "css_selector", "text": "value" }.
    - "click_element" needs { "selector": "css_selector" }.
    - Update your Todo list and Observation log based on what you find.
    - If you see an interesting link or button, try to interact with it.

    RETURN JSON ONLY:
    {
      "thought": "I found a GitHub link, I'm going to click it to see the code.",
      "actionType": "click_element",
      "actionInput": { "selector": "a[href*='github.com']" },
      "newObservations": ["Found GitHub link for Project X"],
      "updatedTodo": ["Explore repository", "Check for install instructions"]
    }
  `;
  ...
  try {
    ...
    console.log(`\x1b[32m[Action]:\x1b[0m ${action.actionType} -> ${JSON.stringify(action.actionInput)}`);
    ...
    state.observationLog.push(`Result of ${action.actionType}: ${toolResult.substring(0, 300)}...`);

    // Short rest to prevent API ban, then loop forever
    setTimeout(startAutonomousLoop, 4000);

  } catch (err) {
    console.error("Critical Error in Loop:", err.message);
    // Restart after 10 seconds if something crashes
