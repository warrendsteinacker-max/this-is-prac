...
// ── Tool 1: Generate HTML report (text topic) ─────────────────────────────────
server.tool(
  "create_html_report",
  "Generates a complete styled HTML report body and style manifesto based on topic, user instructions, and a full style manifest",
  {
    topic:         z.string().describe("The report topic or subject matter"),
    userPrompt:    z.string().optional().describe("Additional user instructions for content, style, or layout"),
    styleManifest: StyleManifestSchema.describe("Full style configuration"),
  },
  async ({ topic, userPrompt = '', styleManifest = {} }) => {
    try {
      const result = await generateReportHtml(topic, userPrompt, styleManifest);
...
// ── Tool 2: Generate HTML report from uploaded file ───────────────────────────
server.tool(
  "create_html_report_from_file",
  "Generates a styled HTML report by reading an uploaded file (PDF, image, or text). Pass the file as base64.",
  {
    fileBase64:    z.string().describe("Base64-encoded file contents"),
    mimeType:      z.string().describe("MIME type, e.g. 'application/pdf', 'image/png', 'text/plain'"),
    originalName:  z.string().describe("Original filename, e.g. 'quarterly-report.pdf'"),
...
// ── Tool 4: Generate + render PDF in one shot ─────────────────────────────────
server.tool(
  "create_and_render_pdf",
  "Generates a styled HTML report from a topic and immediately renders it to PDF — full pipeline in one call",
  {
    topic:         z.string().describe("The report topic"),
    userPrompt:    z.string().optional(),
    styleManifest: StyleManifestSchema,
...
process.stderr.write('     • create_html_report_from_file\n');
process.stderr.write('     • render_pdf\n');
process.stderr.write('     • create_and_render_pdf\n');
process.stderr.write('     • create_and_render_pdf_from_file\n');
process.stderr.write('   Waiting for client via stdio...\n\n');